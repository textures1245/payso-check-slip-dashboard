<script lang="ts">
	...
	async function checkLocalStorageChanges() {
		...
		if (
			...
			currentbranchParam !== previousBranch
		) {
...
<div class="flex justify-between mx-2 my-2">
	<div class="content-center font-semibold">รายการย้อนหลัง</div>
	<div class="flex" style="height: 30px;">
		<select
			bind:value={statusToSend}
			class="border-2 mx-2 rounded-lg border border-stone-400 text-black text-start pl-2"
		>
				...
				<div class="content-center">{item.Status}</div>
							...
							<Table.Row>
								<Table.Cell class="font-medium"
									>{formatDate(item.CreatedAt.split('T')[0])}</Table.Cell
								>
								<Table.Cell class="text-center">{formatTime(item.CreatedAt)}</Table.Cell>
								<Table.Cell class="sm:text-pretty text-center">
								...
								>
								<Table.Cell class=" flex justify-start font-semibold"
									>{#if item.Status === 'SUCCESS'}
										<span class=" flex items-center" style="color:#17B26A;"
											><svg
												class="lg:hidden md:hidden sm:block block"
												...
												viewBox="0 0 120 120"
											>
												<circle cx="60" cy="64" r="48" opacity=".35"></circle><circle
													cx="60"
													cy="60"
													r="48"
													fill="#44bf00"
											...
											<div
												class="lg:block md:block sm:hidden hidden badge badge-outline badge-sm uppercase text-sm lg:flex md:flex p-3"
												style="background-color: #DCFAE6 ;color:#17B26A; border-color:#17B26A"
											>
												SUCCESS
											</div></span
										>
									{:else if item.Status === 'PENDING'}
										<span class=" text-warning flex items-center"
											...
											>
												<g
													...
													><g transform="scale(5.12,5.12)"
														><path
															d="M25,2c-12.683,0 -23,10.317 -23,23c0,12.683 10.317,23 23,23c12.683,0 23,-10.317 23,-23c0,-12.683 -10.317,-23 -23,-23zM25,28c-0.462,0 -0.895,-0.113 -1.286,-0.3l-6.007,6.007c-0.195,0.195 -0.451,0.293 -0.707,0.293c-0.256,0 -0.512,-0.098 -0.707,-0.293c-0.391,-0.391 -0.391,-1.023 0,-1.414l6.007,-6.007c-0.187,-0.391 -0.3,-0.824 -0.3,-1.286c0,-1.304 0.837,-2.403 2,-2.816v-14.184c0,-0.553 0.447,-1 1,-1c0.553,0 1,0.447 1,1v14.184c1.163,0.413 2,1.512 2,2.816c0,1.657 -1.343,3 -3,3z"
														></path></g
													></g
												>
											</svg>
											<div
												class="lg:block md:block sm:hidden hidden badge badge-outline badge-sm uppercase text-sm lg:flex md:flex p-3"
												style="background-color: #FEF0C7;color:#F79009;border-color:#F79009"
											>
												PENDING
											</div></span
										>
				...
				<span class=" text-warning flex  items-center "><svg class="lg:hidden  md:hidden sm:block block" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30" height="30" viewBox="0,0,256,256">
					...
					{:else if item.Status === 'REQUEST_REJECTED'} -->
										<span class=" text-warning flex items-center" style="min-height: 50px;"
											...
											<div
												class="lg:block md:block sm:hidden hidden badge badge-outline badge-sm uppercase text-xs lg:flex md:flex p-3 whitespace-nowrap"
												style="background-color: #F9FAFB ; color: #61646C; border-color: #61646C;"
											>
												Request Rejected
											</div></span
										>
									{:else if item.Status === 'FAILED' || item.Status === 'REQUEST_REJECTED' || item.Status === 'RESPOND_REJECTED'}
										<span class=" flex items-center" style="color:#F04438;"
											><svg
												xmlns="http://www.w3.org/2000/svg"
												class="lg:hidden md:hidden sm:block block"
												x="0px"
											...
											<div
												class="lg:block md:block sm:hidden hidden badge badge-outline badge-sm uppercase text-sm lg:flex md:flex p-3"
												style="background-color: #FEE4E2;color:#F04438;border-color:#F04438"
											>
												FAILED
											</div></span
										>
				...
				<div class="content-center">{item.Status}</div>
							...
							<Table.Row>
								...
								<Table.Cell class="font-medium  text-center"
									>{formatTime(item.CreatedAt)}</Table.Cell
								>

...
<script lang="ts">
	...
	let textClass =
		((pkg.AmountLimit - pkg.OrderAmount) / pkg.AmountLimit) * 100 < 10
			? 'text-red-600'
			: 'text-white';
</script>
...
<Card.Root
	class="relative w-100 rounded-xl overflow-hidden text-center  shadow-none "
	style="height:100%"
>
	<Card.Header
		class={'rounded-t-xl relative overflow-hidden '}
		style="height:170px;background-color:rgba(234, 236, 240, 0.3);"
	>
		<div class="flex justify-center">
			<div
				class="text-xl font-bold uppercase truncate bg-[#F0F4FF] border-[#CCDEFF] border-2 rounded-3xl px-2"
				style="color:#113BD4;"
				title={pkg.Name}
			>
				{pkg.Name}
	...
	<Card.Content class="px-4 py-2 z-0">
		<div class="gap-1 flex text-3xl text-left py-4">
			...
			<div class="text-sm leading-none content-center ml-1">
				{pkg.QuotaLimit.toLocaleString()} สลิป
			</div>
		</div>
	...
	<Card.Footer class="flex-col mt-3 font-semibold ">
		<Button
			...
			style="height:50px"
			on:click={() => sendData(pkg.Price.toFixed(2), pkg.Name, pkg.Id)}
		>
			{pkg.Name === 'Free trial' ? 'ทดลองใช้' : 'ซื้อเลย'}
		</Button>
			...
			<div class="relative w-full mt-3">
				...
				<div class="w-full rounded-full overflow-hidden h-4">
					...
					<progress
						...
						value={pkg.AmountLimit - pkg.OrderAmount}
						max={pkg.AmountLimit}
						style="height: 18px;"
					></progress>
					<!-- Progress bar text -->
					<div
						class="absolute inset-0 flex items-center justify-center {textClass} font-bold text-xs"
					>
	...
	{#if pkg.OrderAmount == maxIndex}
		<div
			class="absolute top-3 right-0 -mr-14 mt-4 transform rotate-45 text-white py-1 px-16 text-xl font-semibold"
			style="background-color: #F04438;color:
#fffff"
		>
			Popular
		</div>

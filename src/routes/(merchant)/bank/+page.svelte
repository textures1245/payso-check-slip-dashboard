	...
	const UpdateBankLink = async (id: String, status: boolean) => {
		...
		try {
			...
			const result = await fetch(
				`${PUBLIC_API_ENDPOINT}/update/bankdata/${id}/${status}/${myCookie.Email}/${type}/${myCookie.Id}`,
				config
			);
	...
	<div class="container max-w-screen-xl pt-5 sm:pt-5 lg:pt-5 mx-auto bg-white rounded-2xl shadow">
		<div class="flex justify-evenly gap-5 mt-5 sm:mt-0 lg:mt-0">
			<div class="w-full sm:w-auto">
				<Card.Root
					class={`w-full min-w-[100px] h-[100px] sm:h-[100px] lg:h-[100px] cursor-pointer 
		...
		{#if isBankSelected}
					...
					<Carousel.Content class="carousel-content flex flex-col">
						{#each banks as banks}
							<Carousel.Item class="flex-shrink-0 h-1/3 mx-2">
								<div class="  p-4 my-2 bg-white border border-gray-200 rounded-lg shadow-md">
									<div class="mt-2">
			...
			<div
				class="relative w-full max-w-[70vw] h-20 sticky bottom-0 mx-auto"
				on:click={() => {
					goto('/banklink');
				}}
			>
				...
				<div class="relative h-full flex items-center justify-center">
					<button
						class="w-full py-2 px-6 flex items-center gap-3 bg-[#ceddf9] hover:bg-[#b5c9ef] transition-colors relative"
					>
						<div class="w-full flex justify-center">
							<div class="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center">
								<Plus class="w-5 h-5 text-white" />
							</div>
							<span class="text-blue-600 font-medium content-center mx-2"
								>สร้างบัญชีธนาคารเพิ่ม</span
							>
						</div>
					</button>
				</div>

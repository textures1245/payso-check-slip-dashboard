<script lang="ts">
	...
	const GetProfile = async () => {
		...
		if (myCookie.Id) {
			...
			url = `http://127.0.0.1:4567/api/v1/merchant/profileid/${myCookie.Id}`;
		} else if (myCookie.Email) {
			console.log('Get by Email');
			url = `http://127.0.0.1:4567/api/v1/merchant/profileemail/${myCookie.Email}`;
		} else {
...
{:else}
	...
	<dialog id="my_modal_2" class="modal">
		<div class="modal-box lg:ml-20">
			...
			<p class=" text-center">Line มีการเชื่อมต่อกับไว้อยู่แล้ว</p>
		</div>
